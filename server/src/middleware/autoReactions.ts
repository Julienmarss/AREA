...
// Initialiser Discord au démarrage
...
// Envoyer notification Discord automatique
...
// Configuration GitHub pour les actions
const GITHUB_CONFIG = {
  TOKEN: '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'
};

// Gestionnaire des interactions Discord (boutons et commandes slash)
...
async function handleCloseIssue(interaction: any, issueNumber: string) {
  ...
  if (response.ok) {
    ...
    const embed = EmbedBuilder.from(interaction.message.embeds[0])
      .setColor(0x6B7280) // gris pour "fermé"
...
async function handleMergePR(interaction: any, prNumber: string) {
  ...
  if (response.ok) {
    ...
    const embed = EmbedBuilder.from(interaction.message.embeds[0])
      .setColor(0x10B981) // vert pour "mergé"
...
// Notification pour Pull Request
async function sendPullRequestNotification(prData: any): Promise<boolean> {
  try {
    ...
    // Extraire les images du corps de la PR (plusieurs formats)
    const images: string[] = [];
    ...
    // Ajouter la première image si elle existe et est valide
    if (images.length > 0) {
      try {
        ...
        // Fallback: Ajouter les images comme liens
        embed.addFields({
          name: `🇮 Images (${images.length})`,
          value: images.slice(0, 3).map((img, i) => `[Image ${i + 1}](${img})`).join(' • '),
          inline: false
...
// Middleware pour intercepter les événements GitHub
export function setupAutoReactions() {
  console.log('🤖 Setting up automatic reactions...');
  
  // Initialiser Discord dès le démarrage
  initDiscordClient().catch(console.error);
  
  return {
    // Fonction appelée pour les issues urgentes
    async handleUrgentIssue(eventType: string, payload: any) {
      ...
      if (eventType === 'issues' && payload.action === 'opened') {
        ...
        if (isUrgent) {
          ...
          // Ajouter bouton de fermeture pour les issues
    ...
    // Fonction appelée pour les Pull Requests
    ...
    // Fonction appelée pour les commentaires
...
// Issue notification avec bouton
async function sendIssueNotificationWithButton(issueData: any, issueNumber: string): Promise<boolean> {
  try {
    ...
    // Extraire les images du corps de l'issue
    const images: string[] = [];
    ...
    // Ajouter la première image si elle existe
    if (images.length > 0) {
      embed.setImage(images[0]);
      if (images.length > 1) {
        embed.addFields({
...
async function sendCommentNotification(commentData: any): Promise<boolean> {
  try {
    ...
    // Extraire images du commentaire
    const images: string[] = [];
    ...
    // Nettoyer et limiter le contenu du commentaire
