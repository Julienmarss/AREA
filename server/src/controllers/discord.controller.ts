...
export class DiscordController {
  ...
  static async callback(req: Request, res: Response) {
    ...
    try {
      ...
      // Utiliser le guild_id du callback si fourni, sinon prendre le premier guild
      const selectedGuildId = (guild_id as string) || guilds[0]?.id;
      
      if (!selectedGuildId) {
        console.error('❌ No guild available');
        const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:8081';
        return res.redirect(`${frontendUrl}/services?error=no_guild`);
      }
      
      // Authentifier l'utilisateur dans le service avec un bot token fictif
      // En production, vous devrez gérer le bot token différemment
      ...
      if (botToken) {
        ...
        if (!success) {
          throw new Error('Discord service authentication failed');
        }
      }
      ...
      console.log(`✅ Discord authenticated for user ${userId} (${discordUser.username}#${discordUser.discriminator})`);
      
      const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:8081';
      return res.redirect(`${frontendUrl}/services?connected=discord`);
